...
import { ensureAmazonQMcpStructure } from './amazonq-utils.js';
import { AmazonQConfigHandler } from './amazonq-config-handler.js';
    ...
    if (matchingDirs.length === 0) {
      ...
      if (officialServerName) {
        try {
          ...
          debugLog(`Removing server "${officialServerName}" from Amazon Q config`);
          configHandler.removeMcpServer(officialServerName);
          configUpdateMsg = `Updated Amazon Q configuration (removed ${officialServerName}).`;
        ...
        // Amazon Q config already updated above
